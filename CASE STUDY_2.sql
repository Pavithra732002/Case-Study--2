...
#EX3
SELECT Borrowing.borrow_date,Borrowing.return_date,members.name, books.title 
FROM Borrowing  JOIN members ON Borrowing.id=members.id JOIN books ON Borrowing.member_id = books.id;

#EX4
SELECT  Members.name, COUNT(Borrowing.id) FROM Members LEFT JOIN Borrowing ON Members.id = Borrowing.member_id GROUP BY Members.id;
